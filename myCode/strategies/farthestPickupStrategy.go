...
func (s *FarthestPickupStrategy) AssignLoadsToDriver(loads []load.Load) [][]int {
    drivers := [][]int{}
    ...
    for len(remainingLoads) > 0 {
        ...
        sort.Slice(remainingLoads, func(i, j int) bool {
			distI := utils.CalculateDistance(currentPosition, remainingLoads[i].End)
			distJ := utils.CalculateDistance(currentPosition, remainingLoads[j].End)
			return distI > distJ
		})
